...
export async function r2Get(key, { raw = false } = {}) {
  ...
  // Sur un gros objet (ex. xml/bib.xml, ~750 Mo), une connexion coupée en
  // cours de route peut clore le flux HTTP proprement (statut 200, 'end'
  // déclenché) sans avoir livré tous les octets — observé en pratique
  // (2026-08-27) : un téléchargement tronqué à ~477 Mo au lieu de ~774 Mo
  // a produit un data/magasins.json silencieusement amputé de 61 953
  // exemplaires de magasin, commité tel quel faute d'erreur levée. Content-
  // Length étant fourni par R2 sur un GET simple (pas de chunked), on peut
  // le comparer à la taille réellement reçue pour détecter ce cas.
  const declaredLength = res.headers['content-length'];
  if (declaredLength !== undefined && res.body.length !== Number(declaredLength)) {
    throw new Error(
      `R2 GET ${key} tronqué : ${res.body.length} octets reçus, ${declaredLength} attendus (Content-Length). ` +
